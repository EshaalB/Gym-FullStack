...
router.get('/revenue-by-type', authenticateToken, requireRole(['Admin']), async (req, res) => {
    try {
...
router.get('/monthly-revenue', authenticateToken, requireRole(['Admin']), async (req, res) => {
    try {
...
router.post('/process', authenticateToken, requireRole(['Admin', 'Trainer']), paymentValidation, handleValidationErrors, async (req, res) => {
    try {
    ...
    try {
        const query = `
            SELECT 
                ...
                p.description
            FROM Payment p
            JOIN gymUser u ON p.userId = u.userId
...
router.patch('/:paymentId/status', authenticateToken, requireRole(['Admin']), async (req, res) => {
    try {
